...
public class SnailFinal : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            //Debug.Log("11");
            Vector2 normal2 = collision.GetComponent<Collider2D>().transform.right;
            Vector2 incomingVector2 = rb.velocity;
            Vector2 reflectVector2 = Vector2.Reflect(incomingVector2, normal2);

            // 设置球的新速度，确保速度大小保持不变
            rb.velocity = reflectVector2.normalized * speedMagnitude;
        }

        if (collision.gameObject.CompareTag("WNHSHU"))
        {
            Debug.Log("11");
            Vector2 normal3 = collision.GetComponent<Collider2D>().transform.right;
            Vector2 incomingVector3 = rb.velocity;
            Vector2 reflectVector3 = Vector2.Reflect(incomingVector3, normal3);
